...
const PUZZLE_CATALOG: Record<MatchPlayablePuzzleType, PuzzleCatalogEntry> = {
  ...
  circuit_clash: { type: "circuit_clash", label: "Circuit Clash", icon: "Bolt", description: "Route power from the source core into every goal node on the board." },
...
function checkPipeConnections(grid: PipeCell[][]) {
  ...
  function flood(row: number, col: number) {
    ...
    if (visited.has(key) || !inBounds(size, row, col)) {
      return;
    }
