...
export default function ProfilePage() {
  ...
  const [socialSource, setSocialSource] =
    useState<GameContentSource>("supabase");
  const [socialResolution, setSocialResolution] =
    useState<GameContentResolution>("unavailable");
  ...
  const [securityQuestionOne, setSecurityQuestionOne] =
    useState(DEFAULT_QUESTION_ONE);
  const [securityQuestionTwo, setSecurityQuestionTwo] =
    useState(DEFAULT_QUESTION_TWO);
  ...
  const worstPuzzleLabel = useMemo(
    () =>
      puzzleTypes.find((entry) => entry.type === user?.worstPuzzleType)
        ?.label ?? "No completed matches yet",
    [puzzleTypes, user?.worstPuzzleType],
  );
  const linkedFacebookPlayers = socialDirectory
    .filter((entry) => entry.facebook_handle)
    .slice(0, 2);
  const linkedTikTokPlayers = socialDirectory
    .filter((entry) => entry.tiktok_handle)
    .slice(0, 2);

  const seasonTier = useMemo(
    () => getPassTierProgress(user?.passXp ?? 0, 40).currentTier,
    [user?.passXp],
  );
  ...
  function isAvatarLocked(optionId: (typeof STOCK_AVATARS)[number]["id"]) {
    ...
    return !(
      Boolean(user?.hasSeasonPass) &&
      seasonTier >= NEON_RIVALS_PREMIUM_AVATAR_TIER
    );
  }
    ...
    try {
      ...
      setProfileStatus(
        canSave
          ...
          : "Guest customization applied locally only.",
      );
    } catch (error) {
      setProfileStatus(
  ...
  return (
    <div className="page-screen">
      <div className="page-stack">
          ...
          right={
            <div className="spotlight-panel flex items-center gap-4">
              <IdentityLoadoutCard
                ...
                subtitle={
                  user?.email ??
                  (accountNeedsSync ? "Profile syncing" : "Guest session")
                }
                avatarId={avatarId}
              ...
              <div className="grid gap-4 sm:grid-cols-[140px_1fr]">
                ...
                <div className="section-stack">
                  <div>
                    ...
                    <p
                      className={`mt-2 font-hud text-[10px] uppercase tracking-[0.2em] ${getRankColor(user?.rank ?? "bronze")}`}
                    >
                      {rankBand.label}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <CosmeticPreview
                      kind="theme"
                      productId={user?.themeId}
                      className="profile-cosmetic-preview"
                    />
                    <CosmeticPreview
                      kind="player_card"
                      productId={user?.playerCardId}
                      className="profile-cosmetic-preview"
                    />
                    <CosmeticPreview
                      kind="banner"
                      productId={user?.bannerId}
                      className="profile-cosmetic-preview"
                    />
                    <CosmeticPreview
                      kind="emblem"
                      productId={user?.emblemId}
                      className="profile-cosmetic-preview"
                    />
                    <CosmeticPreview
                      kind="title"
                      productId={user?.titleId}
                      className="profile-cosmetic-preview"
                    />
                  </div>
                </div>
          ...
          <section className="section-panel">
                  ...
                  {contentError ? (
                    ...
                    <div className="command-panel-soft flex min-h-[180px] items-center justify-center p-6 text-sm text-muted-foreground">
                      {leaderboardResolution === "empty"
                        ? "Live leaderboard entries will appear once ranked results land."
                        : leaderboardResolution === "unavailable"
                          ? "Live leaderboard data is currently unavailable."
                          : "Leaderboard data is unavailable in this environment."}
                    </div>
                  )}
                </div>
              </>
            ...
            {tab === "social" && (
              <>
                ...
                <div className="section-stack">
                  {!isSupabaseConfigured ? (
                    <div className="command-panel-soft px-4 py-3 text-sm text-destructive">
                      {supabaseConfigErrorMessage}
                    </div>
                  ) : null}
                  {isGuest ? (
                    <div className="command-panel-soft p-4 text-sm leading-6 text-muted-foreground">
                      Use the sign-in or sign-up flow first. Facebook and TikTok
                      linking unlock once the account is live.
                    ...
                    <>
                      <div className="action-grid">
                        <PuzzleTileButton
                          ...
                          disabled={
                            isWorking || user?.linkedProviders?.facebook
                          }
                        />
                        <PuzzleTileButton
                          icon={Link2}
                          title={
                            user?.linkedProviders?.tiktok
                              ? "TikTok linked"
                              : "Link TikTok"
                          }
                  ...
                  <div className="deck-grid">
                    {[...linkedFacebookPlayers, ...linkedTikTokPlayers].slice(
                      0,
                      4,
                    ).length > 0 ? (
                      [...linkedFacebookPlayers, ...linkedTikTokPlayers]
                        .slice(0, 4)
                        .map((entry) => (
                          <div
                            key={entry.id}
                            className="command-panel-soft flex items-center gap-3 p-4"
                          >
                            <StockAvatar
                              avatarId={entry.avatar_id ?? DEFAULT_AVATAR_ID}
                              size="sm"
                            />
                      ...
                      <div className="command-panel-soft flex min-h-[140px] items-center justify-center p-6 text-sm text-muted-foreground">
                        {isContentLoading
                          ? "Loading social directory..."
                          : socialResolution === "empty"
                            ? "No live linked player identities are available yet."
                            : socialResolution === "unavailable"
                              ? "Live social directory data is currently unavailable."
                              : "Social directory data is unavailable in this environment."}
                      </div>
                    )}
                  </div>
                </div>
            ...
            {tab === "security" && (
              <>
                ...
                <div className="section-stack">
                  <div className="deck-grid">
                    <div className="command-panel-soft p-4">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setSecurityQuestionOne(event.target.value)
                        }
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSecurityAnswerOne(event.target.value)
                        }
                        className="mt-3 h-12 w-full rounded-2xl border border-border bg-background/35 px-4 text-sm outline-none focus:border-primary"
                        placeholder="Answer"
                        type="password"
                    ...
                    <div className="command-panel-soft p-4">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setSecurityQuestionTwo(event.target.value)
                        }
                        className="mt-3 h-12 w-full rounded-2xl border border-border bg-background/35 px-4 text-sm outline-none focus:border-primary"
                      >
                        {SECURITY_QUESTION_OPTIONS.map((question) => (
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setSecurityAnswerTwo(event.target.value)
                        }
                        className="mt-3 h-12 w-full rounded-2xl border border-border bg-background/35 px-4 text-sm outline-none focus:border-primary"
                        placeholder="Answer"
                        type="password"
                  ...
                  <Button
                    onClick={() => void handleSaveSecurityQuestions()}
                    variant="play"
                    size="xl"
                    className="w-full"
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-xs font-black text-primary">
                              {entry.isRead
                                ...
                                : `New | ${formatActivityTime(entry.occurredAt)}`}
                            </p>
                          </div>
                        }
                      />
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-6 text-muted-foreground">
                          {activityResolution === "empty"
                            ? canSave
                              ? "Your live activity feed is ready but empty right now. Match results, purchases, and social updates will appear here automatically."
                              : "Sign in to start a live activity feed across devices."
                            : activityResolution === "unavailable"
                              ? "Live activity data is currently unavailable."
                              : "Activity data is unavailable in this environment."}
                        </p>
                      </div>
                    </div>
                  )}
        ...
        {accountStatus ? (
          <p className="px-1 text-sm text-muted-foreground">{accountStatus}</p>
        ) : null}
      </div>
    </div>
  );
